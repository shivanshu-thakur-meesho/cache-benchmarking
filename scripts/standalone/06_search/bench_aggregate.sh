...
# --- GROUPBY + COUNT ---
run_memtier "standalone_search_agg_count" \
...
# --- GROUPBY + AVG ---
run_memtier "standalone_search_agg_avg" \
  docker run --rm --network=host \
  --cpus="$CPUS" \
  --memory="${HALF_MEM_KB}k" \
  --memory-swap="${HALF_MEM_KB}k" \
...
# --- GROUPBY + SUM + LIMIT ---
run_memtier "standalone_search_agg_sum" \
  docker run --rm --network=host \
  --cpus="$CPUS" \
  --memory="${HALF_MEM_KB}k" \
  --memory-swap="${HALF_MEM_KB}k" \
  ...
  --clients="$CPUS" \
  --pipeline="$PIPELINE" \
  --command="FT.AGGREGATE product_idx * GROUPBY 1 @category REDUCE SUM 1 @stock AS total_stock SORTBY 2 @total_stock DESC LIMIT 0 5" \
  --command-ratio=1 \
  --command-key-pattern=R \
  --test-time="$TEST_TIME" \
  --hide-histogram
