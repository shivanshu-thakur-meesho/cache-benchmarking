#!/usr/bin/env bash
# Search Benchmark: FT.SEARCH with filters (cluster)
...
echo ""
echo -e "${BOLD}Running FT.SEARCH filter benchmarks (cluster)...${NC}"
echo ""

run_search_bench "cluster_search_numeric_range" \
  FT.SEARCH product_idx "@price:[10 50]" LIMIT 0 10

run_search_bench "cluster_search_tag_filter" \
  FT.SEARCH product_idx "@category:{electronics}" LIMIT 0 10

run_search_bench "cluster_search_combined_filter" \
  FT.SEARCH product_idx "@category:{electronics} @price:[0 100]" LIMIT 0 10

run_search_bench "cluster_search_sorted" \
  FT.SEARCH product_idx "*" SORTBY price ASC LIMIT 0 10

run_search_bench "cluster_search_nocontent" \
